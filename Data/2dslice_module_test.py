...
with open(os.path.join(Path(__file__).resolve().parents[1], os.path.join("Robot", os.path.join("UploadFolder", "scan_data_time_7.2.csv"))), newline='') as f:
    reader = csv.reader(f)
...
plt.figure()
plt.imshow(pseudo_signal.distance_responses, vmin=0, vmax=np.abs(pseudo_signal.distance_responses).max(), aspect='auto', extent=[0,360,pseudo_signal.distance_responses.shape[0]*pseudo_signal.distance_time_scale,0], cmap='jet')
